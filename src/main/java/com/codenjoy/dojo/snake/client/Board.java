...
public class Board extends AbstractBoard<Elements> {
    ...
    private static boolean searchBadApple = false;
    ...
    public String getNextDirection() {
        ...
        if (searchBadApple) {
            if (snakeSize > snake.size()) {
                searchBadApple = false;
            }
        }

        Elements goalStone = searchBadApple
                ? Elements.BAD_APPLE
                : Elements.GOOD_APPLE;
        Elements badStone = searchBadApple
                ? Elements.GOOD_APPLE
                : Elements.BAD_APPLE;
        Point goal = searchBadApple ? getStones().get(0) : getApples().get(0);
        ...
        List<Point> path = DirectionUtils.leePath(head, goal, barriers);
//        List<Point> path = DirectionUtils.findPath(head, goal, barriers);
        ...
        Direction direction = DirectionUtils.findSaveDirection(head, barriers);
